...
def get_frames_data(filename, num_frames_per_clip=16 ):
  ...
  for parent, dirnames, filenames in os.walk(filename):
    # print(len(filenames))
    if( len(filenames) < 2 * num_frames_per_clip + 1 ):   ###
      return [], s_index
    filenames = sorted(filenames)
 
    #s_index = random.randint(0, len(filenames) - int(num_frames_per_clip))
    s_index = 1
...
def read_clip_and_label(dataset_dir, filename, batch_size, start_pos=-1, num_frames_per_clip=16, crop_size=112, shuffle=False):
  ...
  dataset_file_path=filename.split('label')[0]+'dataset/'
